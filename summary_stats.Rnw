...
\subsubsection{Summary Statistics about Trips}
...
In the \var{trips} dataset, 15 variables are extracted from the original data and the last 
\var{variable} weekday is added to indicate the index of the day in a week for 
...
We aggregate the trips by station pairs and find that the top four popular route are round trips near central park area. As shown in the figure, we plot the stations that are shown in the top ten popular trip pairs. (Figure \ref{fig:popular-stations}) 
\begin{figure}
  \includegraphics[width=\textwidth]{popular_stations.png}
