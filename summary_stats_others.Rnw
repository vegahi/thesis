...
\subsubsection{Others Summary Statistics}
...
Given the total number of trips per bike, we could see that there's a clear 
descending trend for bikes with ID number above 20000. We would like to check if 
that's related how long they have been in use. (Figure~\ref{fig:bike-id})
...
For each bike, plot the total number of trips against the first time it is in 
use. (Figure \ref{fig:bike-firstuse})
...
\subsubsubsection{User Type}

Breaking down the user type information, we could see that most of the 
information is available. Among that, 0.12 are customers who have 24-hour pass 
or 7-day pass and 0.88 are subscribers who are annual members.
...
distribution is plotted. We could see  most of the majority of users are born 
after 1975. (Figure \ref{fig:birth-year})
...
For the gender column (Zero=unknown; 1=male; 2=female), around 0.12 of the users 
are unknown, 0.67 of the users are male and 0.21 are female.
